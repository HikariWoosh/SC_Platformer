...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField]
    private bool onGround; // Bool to decide if the player is grounded

    [SerializeField]
    private bool sceneLoaded; // Checks the scene is fully loaded
    ...
    [SerializeField]
    private float cooldownTimer; // Timer until the player can dash again
    ...
    [SerializeField]
    private GameObject UI; // Reference to the UI object

    [SerializeField]
    public Image StelCrystal; // Reference to the image used for dashing
    ...
    [SerializeField]
    public bool Movement2D; // Bool to control movement depending on camera state
    ...
    private float capsuleRadius; // Radius of the capsule collider

    private LayerMask CheckpointsLayer; // Reference to the CheckpointsLayer
    ...
    // Used to force the player in a certain direction
    ...
    // Function run to cause the player to dash
    public void Dash()
    {
    ...
    // Coroutine to manage the dashing action
    ...
    // Coroutine to manage the players dash cooldown
    IEnumerator DashCooldown()
    {
        cooldownTimer = 0f;

        // Run to check if the user is grounded or ariel and manage the cooldown correctly (Wavedashing)
        StartCoroutine(DashCooldownGrounded());

        yield return new WaitForSeconds(dashCooldown);

        // Used to recharge the cooldown
    ...
    // Resets thep players dash when grounded
    IEnumerator DashCooldownGrounded()
    {
        if (!onGround)
        {
            // Waits until the player is grounded
